...
<%
	...
	if(where.equals("where category2='null'")){where="where category2='"+category+"'";}
	
	List<DB_data> list = DB.eachlist(order, startRow, listSize, where);
	int count=DB.count();  
...
<div id="mid"> <!-- width:1600, padding:300 -->
	<p id="subject"><%=category%><p>
	<div id="list">
		<%
		for(int i=0; i<hash.length; i++){
		%><a href="IKEA_COM.jsp?pageName=new_product_each.jsp&where=<%=hash[i]%>&category=<%=hash[i]%>"><span><%=hash[i]%></span></a>
...
%>		<a href="IKEA_COM.jsp?pageName=view_product.jsp&itemno=<%=list.get(j).getItemno()%>">
			<div id="product_view_mouse<%=j+1%>" class="product_view_mouse"  onmouseleave="test<%=j+101%>()">
				...
				<a href=""><span class="product_view_mouse_get">구매하기</span></a><br><br>
				<a href=""><span class="product_view_mouse_get">장바구니</span></a>
			</div></a>
