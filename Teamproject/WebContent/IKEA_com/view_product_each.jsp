...
<div id="main">
	...
	<div id="item_container">
		...
		<div id="itemoption1">
			<button id="chg" name="itemoption1" onclick="itemoption1()">옵 션Ⅰ :  <%=data.getItemshape1() %></button>
			<ul id="itemoption11" class="itemoption11">
				<%for(int i=0; i<shape.size();i++){%>
					<li onclick="optchg<%=i+1%>()"><span class="opimg"><img src="/../img_item/<%=data.getImage1()%>"/></span>
				 	<div id="optchg<%=i+1%>"><%= shape.get(i) %></div></li>  
				<%} %>	
			</ul>
		</div>
		<div id="itemoption2">
			<button id="chgg" name="itemoption2" onclick="itemoption2()">옵 션Ⅱ :  <%=data.getItemsize1() %></button>
			<ul id="itemoption22" class="itemoption22">	
				<%for(int i=0; i<size.size();i++){%>
					<li onclick="optchgg<%=i+1%>()"><span class="opimg"><img src="/../img_item/<%=data.getImage1()%>"/></span>
		...
		<div id="product_count">
			...
			<script>
				function addwish(){
					...
					if(custno!=null){
						...
						var shape=document.getElementById("chg").innerHTML;
						shape=shape.replace("옵 션Ⅰ :  ","")
						var size=document.getElementById("chgg").innerHTML;
						size=size.replace("옵 션Ⅱ :  ","")
			...
			<span>수량 : </span><input type="text" value="1" id="item_count" name="item_count" onchange="itemcount()">
		</div>
		<div id="product_buy">
